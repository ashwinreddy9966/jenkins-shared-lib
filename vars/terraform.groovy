def call() {
    ...
    properties([
            parameters([
                    ...
                    string(choices: 'APP_VERSION',description: 'Choose App Version To Deploy', name: "APP_VERSION")
            ]),
    ])
    node {
        ansiColor('xterm') {
            ...
            stage('Terraform Apply') {
                ...
                cd ${TERRAFORM_DIR}
                export TF_VAR_APP_VERSION=${APP_VERSION}
                terraform apply -auto-approve -var-file=env-${ENV}/${ENV}.tfvars -var APP_VERSION=${APP_VERSION}
                '''
            }
        }
    }
